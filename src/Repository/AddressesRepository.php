...
class AddressesRepository extends ServiceEntityRepository {
	...
	public function findInMultipleFields($value, $user) {
		return $this->createQueryBuilder('a')
			->andWhere('a.name LIKE :val OR a.surname LIKE :val OR a.address LIKE :val')
			->andWhere('a.owner = :bal')
			->setParameter('val', '%' . $value . '%')
			->setParameter('bal', $user)
			->getQuery()
			->getResult()
		;
	}
